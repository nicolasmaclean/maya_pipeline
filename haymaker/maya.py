...
class SurfacingUtils:
    @staticmethod
    @preserve_selection
    def get_material_assignments(obj, remove_namespaces=True):
        """
        ...
        for group in shading_groups:
            ...
            for members in cmds.sets(group, q=True):
                for member in cmds.ls(members, long=True):
                    ...
                    i = member.find('.f[')
                    member_without_faces = member if i < 0 else member[:i]
                    if member_without_faces not in obj_and_descendents:
                        continue

                    if remove_namespaces:
                        # look at each object of the full object path
                        # remove the namespace (if there is one)
                        # put it all back together
                        #   do this without the face-set, then add the face set-back
                        faces = '' if i < 0 else member[i:]
                        member = '|'.join([
                            o.split(':')[-1]
                            for o in member_without_faces.split('|')]
                        ) + faces
    ...
    def set_material_assignments(obj, assignments):
        ...
        obj_without_namespace = obj.strip('|').split(':')[-1]
        for shading_group, members in assignments.items():
            qualified_members = []
            for member in members:
                # make sure the member is under the root object
                member_heir = member.strip('|').split('|')
                if member_heir[0] != obj_without_namespace:
                    continue

                # get the full object path with namespaces
                found = False
                qualified_member = obj
                for o in member_heir[1:]:
                    i = o.find('.f[')
                    o_without_faces = o if i < 0 else o[:i]
                    faces = '' if i < 0 else o[i:]
                    found = False
                    children = cmds.listRelatives(qualified_member, children=True)
                    if not children:
                        break
                    for child in children:
                        if child.split(':')[-1] == o_without_faces:
                            qualified_member += f'|{child}{faces}'
                            found = True
                    if not found:
                        break
                if not found:
                    log(f'Unable to find {member} to apply {shading_group}')
                    continue
